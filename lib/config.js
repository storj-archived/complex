 ...
 * @param {String} [options.networkHDIndex] - The node index for the renter
...
function RenterConfig(options) {
  ...
  this._.networkHDPrivateKey = options.networkHDPrivateKey;
  this._.networkHDIndex = options.networkHDIndex;
...
function readPrivateKey(config) {
  if (config._.networkHDPrivateKey) {
    config._.networkHDPrivateKey = fs.readFileSync(
      config._.networkHDPrivateKey
    ).toString('utf8').split('\n').join('');
  }
  if (config._.networkPrivateKey) {
