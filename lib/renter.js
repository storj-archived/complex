...
Renter.METHOD_MAP_REDIRECT = {
  getStorageOffer: Renter.prototype._getStorageOffers
};
...
Renter.prototype._handleWork = function(buffer) {
  ...
  try {
    if (Renter.METHOD_MAP_REDIRECT[data.method]) {
      Renter.METHOD_MAP_REDIRECT[data.method].apply(
        this,
...
Renter.prototype._getStorageOffers = function(contract, blacklist, callback) {
  ...
  function _handleError(err) {
    self._logger.error('error occurred while processing offers for %s', hash);
  }
};
