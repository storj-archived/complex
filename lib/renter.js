...
Renter.prototype._initPrivateKey = function(options) {
  assert(options.networkPrivateExtendedKey || options.networkPrivateKey,
         'networkHDPrivateKey or networkPrivateKey is expected');
  ...
  if (options.migrationPrivateKey) {
    this.migrationKeyPair = new storj.KeyPair(options.migrationKeyPair);
  }
...
Renter.prototype.start = function(callback) {
  ...
  this._loadKnownSeeds(function(err, seeds) {
    ...
    this.network = storj.RenterInterface({
      ...
      keyPair: this.hdKey ? undefined : this.keyPair,
      hdKey: this.hdKey ? this.hdKey.privateExtendedKey : undefined,
    ...
    case 'getMirrorNodes':
      args[0] = args[0].map(function(pointerData) {
        return storj.DataChannelPointer(
          storj.Contact(pointerData.farmer),
          pointerData.hash,
          pointerData.token,
...
Renter.prototype._getRetrievalPointer = function(contact, contract, callback) {
  ...
  if (!this.migrationKeyPair || contract.get('renter_hd_key')) {
    return this.network.getRetrievalPointer.call(this.network, arguments);
  }
  ...
  this._renewContract(contact, contract, function(err) {
    ...
    self.network.getRetrievalPointer.call(self.network, arguments);
  });
};
...
Renter.prototype._renewContract = function(contact, contract, callback) {
  ...
  updatedContract.set('renter_hd_key', this.hdKey.publicExtendedKey);
  updatedContract.set('renter_hd_index', this.hdIndex);
  updatedContract.set('renter_id', this.keyPair.getNodeID());
  updatedContract.sign('renter', this.keyPair.getPrivateKey());

  var renewRpc = new kad.Message({
    method: 'RENEW',
    params: {
  ...
  this.network.transport.send(contact, renewRpc, function(err, message) {
    ...
    var farmerSignedContract = new storj.Contract(message.result.contract);

    // TODO verify the farmer signed contract
    ...
    self.network.storageManager.load(dataHash, function(err, item) {
      ...
      item.removeContract({ nodeID: contract.get('farmer_id') });
      item.addContract(
        { nodeID: contract.get('farmer_id') },
        farmerSignedContract
      );
