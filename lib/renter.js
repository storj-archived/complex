 ...
 * @param {String} [options.networkHDPrivateKey] - The extended private key
 * @param {Number} [options.networkHDIndex] - The renter index for extended key
...
function Renter(options) {
  ...
  options = this._initPrivateKey(options);
...
Renter.prototype._initPrivateKey = function(options) {
  assert(options.networkHDPrivateKey || options.networkPrivateKey,
         'networkHDPrivateKey or networkPrivateKey is expected');
  if (options.networkHDPrivateKey) {
    assert(storj.utils.isValidNodeKey(options.networkHDPrivateKey),
           'networkHDPrivateKey is expected to be an extended private key');
    assert(storj.utils.isValidNodeIndex(options.networkHDIndex),
           'networkHDIndex is expected to be non-hardened derivation index');
    assert(!options.networkPrivateKey,
           'networkPrivateKey is NOT expected with networkHDPrivateKey');
    var hdkey = HDKey.fromExtendedKey(this._opts.networkHDPrivateKey);
    var child = hdkey.deriveChild(this._opts.networkHDIndex);
    var privateKey = child.privateKey;
    assert(privateKey, 'Unable to derive privateKey');
    options.networkPrivateKey = privateKey;
  }
  return options;
};
...
Renter.prototype._signStorageContract = function(contract) {
  var keyPair = storj.KeyPair(this._opts.networkPrivateKey);
  if (this._opts.networkHDPrivateKey) {
    var hdkey = HDKey.fromExtendedKey(this._opts.networkHDPrivateKey);
    contract.set('renter_hd_key', hdkey.publicExtendedKey);
    contract.set('renter_hd_index', this._opts.networkHDIndex);
  }
  contract.set('renter_id', keyPair.getNodeID());
