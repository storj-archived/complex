...
Renter.prototype._handleRouterEvent = function(data) {
  ...
  switch (event.type) {
    case 'add':
      ...
      this.network.router.updateContact(event.data);
      break;
    case 'drop':
      this._logger.debug('dropping contact from pool event');
      this.network.router.removeContact(event.data);
      break;
    case 'shift':
    default:
...
Renter.prototype._handleNetworkEvents = function() {
  ...
  // Record contacts that we come across
  function _recordContactSeen(contact) {
    self.storage.models.Contact.record(contact);
  }

  function _onContactAdded(contact) {
    self.routerPub.write(Buffer(JSON.stringify({
      type: 'add',
      data: contact
    })));
    _recordContactSeen(contact);
  }

  function _onContactDropped(contact) {
    self.routerPub.write(Buffer(JSON.stringify({
      type: 'drop',
      data: contact
    })));
  ...
  this.network.router.on('add', _onContactAdded);
  this.network.router.on('drop', _onContactDropped);
  this.network.router.on('shift', _recordContactSeen);
