...
Renter.prototype._initNetwork = function(seeds) {
  ...
  this.network.on('unhandledOffer', (contact, contract, callback) => {
    this._storeAsQueuedMirror({ contact, contract }, callback);
  });
};
...
/**
 * Stores the received offer as a mirror document
 * @private
 * @param {object} offer
 * @param {object} offer.contact
 * @param {object} offer.contract
 * @param {function} callback
 */
Renter.prototype._storeAsQueuedMirror = function(offer, callback) {
  const self = this;
...
Renter.prototype._getStorageOffers = function(contract, blacklist, callback) {
  ...
  var mirrorStream = self.storage.models.Mirror.find({ shardHash: hash }).cursor();
  var didReceiveFirstOffer = false;

  function _handleOffer(offer) {
    mirrorStream.removeAllListeners();
    mirrorStream.close();
    offerStream.pause();

    if (didReceiveFirstOffer) {
      return self._storeAsQueuedMirror(offer, () => offerStream.resume());
    }
  ...
  mirrorStream.once('data', (mirror) => {
    _handleOffer({
      contact: mirror.contact,
      contract: new storj.Contract(mirror.contract)
    });
  });
