...
Renter.prototype._publishContract = function(contacts, contract, callback) {
  ...
  // TODO setup offer stream to handle OFFER messages and call the callback
  // with an available offer.

  async.each(contacts, (contact, next) => {

    // Update the contact that we sent a PUBLISH message
    contact.lastPublishSent = Date.now();
    contact.save((err) => {

      // Send the node a contract directly
      this.network.publishContract(contact, contract, (err) => {
        if (err) {
