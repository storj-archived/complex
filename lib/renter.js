...
const kad = storj.deps.kad;
const MongoStorageAdapter = require('storj-mongodb-adapter');
const HDKey = require('hdkey');
const ReadableStream = require('readable-stream');
const inherits = require('util').inherits;
const Logger = require('kad-logger-json');
const RenterConfig = require('./config').RenterConfig;
...
Renter.prototype._handleWork = function(sock, buffer) {
  ...
  switch (data.method) {
    ...
    case 'getStorageProof':
      nodeID = data.params[0].nodeID;
      dataHash = data.params[1].data_hash;
      break;
    case 'getStorageOffer':
      dataHash = data.params[0].data_hash;
      break;
  }

  this._logger.info('running job, method: %s, id: %s, ' +
                   'data_hash: %s, node_id: %s',
                   data.method, data.id, dataHash, nodeID);
...
Renter.prototype._recordResponseTime = function(nodeID, repPoints, responseTime) {
  this.storage.models.Contact.findOne({_id: nodeID}, (err, contact) => {
    if (err || !contact) {
      this._logger.warn('_recordResponseTime: Error trying to find contact ' +
    ...
    contact.recordResponseTime(responseTime).save((err) => {
      if (err) {
        this._logger.warn('_recordResponseTime: Unable to save contact %s ' +
...
Renter.prototype._publishContract = function(contacts, contract, callback) {
  const self = this;
  let callbackCalled = false;

  const contractInst = storj.Contract(contract);
  this._signStorageContract(contractInst);

  async.each(contacts, (contact, next) => {

    // Update the contact last contract sent time for load balancing
    this.storage.models.Contact.updateLastContractSent(contact._id, (err) => {
      if (err) {
        return self._logger.error('Unable to update contact last sent time, ' +
                                  'reason: %s', err.message);
      }

      // TODO try/catch
      const contactInst = storj.Contact({
        nodeID: contact._id,
      ...
      // TODO attach callback for timeout event on the request
      this.network.publishContract(contactInst, contractInst, (err, data) => {
        const responseTime = Date.now() - startTime;
        if (err) {
          this._recordResponseTime(nodeID, points.REQUEST_ERROR, responseTime);

          self._logger.warn('Error publishing contract to node: %j, reason: %s',
                            contact, err.message);
        }

        if (data && data.result) {
          const token = data.result.token;
          const farmerContract = data.result.contract;
          const farmerContact = data.result.contact;

          const nodeID = contact._id;

          this._recordResponseTime(nodeID, points.REQUEST_SUCCESS, responseTime);

          self.storage.models.Mirror.createWithToken(
            farmerContract,
            farmerContact,
            token,
            function(err) {
              ...
              // Send back the first offer that is received
              if (!callbackCalled) {
                callbackCalled = true;
                callback(null, {
    ...
    if (!callbackCalled) {
      callback(new Error('Unable to get offer'));
    }
  });
