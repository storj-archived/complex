...
Renter.prototype._publishContract = function(contacts, contract, callback) {
  ...
  async.each(contacts, (contact, next) => {
    ...
    this.storage.models.Contact.updateLastContractSent(contact._id, (err) => {
      if (err) {
        return log.error('Unable to update contact last sent time, reason: %s',
                         err.message);
      }

      // TODO try/catch
      const contractInst = storj.Contact(contact);
      const contractInst = storj.Constract(contract);

      // Send the node a contract directly
      this.network.publishContract(contactInst, contractInst, (err, offer) => {
        if (err) {
          log.warn('Error publishing contract to node: %j', contact);
        }
